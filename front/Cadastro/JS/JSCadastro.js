document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function loadTranslations(lang) {
    try {
      ...
      showFeedback(
        "error",
        `Erro ao carregar idioma ${lang}. Verifique se o arquivo ${lang}.json existe.`
      );
  ...
  form.addEventListener("submit", async function (event) {
    ...
    try {
      ...
      const response = await fetch(API_URL, { "/usuarios/novoUsuario": json });
      if (!response.ok) {
      ...
      if (response.status === 201 && responseData.id) {
        ...
        showFeedback(
          "warning",
          responseData.message || "feedback-register-warning"
        );
      }
    } catch (error) {
      ...
      if (
        ...
        showFeedback("error", "error-server-generic", {
