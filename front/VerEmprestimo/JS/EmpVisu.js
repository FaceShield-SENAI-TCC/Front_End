        ...
        function loadLoans() {
            ...
            paginatedLoans.forEach(emp => {
                ...
                row.innerHTML = `
                    ...
                    <td>${usuario.turma}</td>
                    <td>${ferramenta.nome} <br><small>${ferramenta.marca} ${ferramenta.modelo}</small></td>
                    <td>${formatLocation(ferramenta.local)}</td>
                    <td>${formatDateTime(emp.data_retirada)}</td>
                    <td>${emp.data_devolucao ? formatDateTime(emp.data_devolucao) : '-'}</td>
                    <td>
        ...
        function filterLoans() {
            ...
            return emprestimos.filter(emp => {
                ...
                if (status === 'delayed') {
                    ...
                    if (!emp.data_prevista || new Date() <= new Date(emp.data_prevista)) return false;
                }
